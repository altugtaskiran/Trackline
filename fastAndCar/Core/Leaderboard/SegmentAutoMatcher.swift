...
//  Entirely opt-in and best-effort — silent no-op when the feature flag is
//  off, the user hasn't opted in from Settings, or CloudKit isn't reachable
//  (offline, not signed into iCloud). Never blocks or surfaces errors to
//  the trip-save flow that calls it.
import Foundation

enum SegmentAutoMatcher {
    static func run(for trip: Trip) async {
        guard FeatureFlags.globalLeaderboardEnabled else { return }
        guard UserDefaults.standard.bool(forKey: "globalLeaderboardOptIn") else { return }
